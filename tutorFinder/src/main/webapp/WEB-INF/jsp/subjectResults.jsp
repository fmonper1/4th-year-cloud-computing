...
<t:template>
    <jsp:body>
        ...
        <c:if test="${empty(subjectResults)}">
            <strong class="text-gray-dark">Sorry no results were found for this subject</strong>
            <br>
            <br>
            <br>
        </c:if>
        <c:if test="${not empty(subjectResults)}">
            <c:forEach  items="${subjectResults}" var="subject">
                <div class="d-flex align-items-center p-3 my-3 text-white-50 bg-purple rounded shadow-sm">
                    <img class="mr-3" src="<c:url value="/resources/img/subject.png" />"  alt="" width="48" height="48">
                    <div class="lh-100">
                        <h6 class="mb-0 text-white lh-100">Results for Tutors who teach ${subject.name}</h6>
